...
_calculate_levd_geodesic_impl(const std::string &s1, const std::string &s2) {
  const size_t n = s1.length();
  const size_t m = s2.length();
...
int calculate_levd_cpp_int(const char *s1_str, const char *s2_str) {
  return _calculate_levd_template<int>(std::string(s1_str),
                                       std::string(s2_str));
}

// Integer version of the distance matrix calculation
void calculate_levd_distance_matrix_cpp_int(const char **sequences, int n,
...
int calculate_levd_geodesic_cpp(const char *s1_str, const char *s2_str,
                                char *buffer, int buffer_len) {
  std::pair<int, std::string> result =
      _calculate_levd_geodesic_impl(std::string(s1_str), std::string(s2_str));
