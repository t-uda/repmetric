...
class GeodesicPath:
    def __init__(self, start_string: str, end_string: str, operations: List[str], distance: int):
    ...
    def get_path_strings(self) -> List[str]:
        ...
        i = 0
        j = 0
        current_s = list(self.start_string) # Mutable
        # But wait, inserting/deleting changes indices.
        # If we use the "processed_prefix + remaining_suffix" approach, we don't need to track indices in `current_s`, only in `start_string` and `end_string`.
        
        processed_prefix = []
        remaining_suffix_idx = 0 # Index in start_string
